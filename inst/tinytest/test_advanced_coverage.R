...
message("Advanced coverage: prepare_concentration, correlate edge cases, plot_correlations full run")
...
if (!file.exists(conc_fixture)) {
  conc_fixture <- file.path(orig_dir, "inst", "tinytest", "fixtures",
...
withr::with_tempdir({
  ...
  utils::write.table(as.data.frame(profiles), profiles_path,
...
withr::with_tempdir({
  corr <- data.frame(
    id_ion = 101L, id_taste = "BITTER",
    fractions = "32 33 34", correlation = 0.99,
    p_value = 0.001, p_adjusted = 0.01, method = "kendall",
  ...
  num_cols <- c(
    "feature_mz", "feature_rt", "score_input",
    "candidate_count_similarity_peaks_matched",
    "candidate_score_similarity",
    "score_initial", "score_biological", "score_chemical", "score_final",
    "feature_pred_tax_npc_01pat_score", "feature_pred_tax_npc_02sup_score",
    "feature_pred_tax_npc_03cla_score"
  )
  all_cols <- c(
    "feature_id", "feature_mz", "feature_rt", "score_input",
    ...
    "candidate_count_similarity_peaks_matched", "candidate_score_similarity",
    "score_initial", "score_biological", "score_chemical", "score_final",
    "feature_pred_tax_npc_01pat_val", "feature_pred_tax_npc_02sup_val",
    "feature_pred_tax_npc_03cla_val",
    "feature_pred_tax_npc_01pat_score", "feature_pred_tax_npc_02sup_score",
    "feature_pred_tax_npc_03cla_score", "mode"
  )
  ann <- as.data.frame(setNames(lapply(all_cols, function(x) {
    if (x == "feature_id") 101L
    else if (x %in% num_cols) 0.95
    else NA_character_
  }), all_cols))
  ann$mode <- "pos"
  ...
  utils::write.table(corr, corr_path, sep = "\t", row.names = FALSE, quote = FALSE)
  utils::write.table(ions, ions_path, sep = ",",  row.names = FALSE, quote = FALSE)
  utils::write.table(ann,  ann_path,  sep = "\t", row.names = FALSE, quote = FALSE)
  ...
  ret <- tryCatch(
    plot_correlations(
      ...
      output_1 = c1, output_2 = c2, output_3 = c3,
      widths = 2:3,
      min_corr = 0.0,
      max_pval = 1.0,
